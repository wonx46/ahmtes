 ...
 * @author achmad.ha
 */
public class PlmHibernateDao <T, K extends Serializable> implements PlmDao<T, K>{
    
    @SuppressWarnings("unchecked")
    protected Class persistentClass;

    @Autowired
    @Qualifier("sessionFactoryPlm")
    protected SessionFactory sessionFactoryPlm;

    @SuppressWarnings("unchecked")
    public PlmHibernateDao() {
        persistentClass = (Class<T>) ((ParameterizedType) getClass().getGenericSuperclass()).getActualTypeArguments()[0];
    }

    protected Session getCurrentSession() {
        return sessionFactoryPlm.getCurrentSession();
    }
