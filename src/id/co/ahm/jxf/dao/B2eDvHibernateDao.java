 ...
 * @author achmad.ha
 */
public class B2eDvHibernateDao <T, K extends Serializable> implements B2eDvDao<T, K>{
    @SuppressWarnings("unchecked")
    protected Class persistentClass;

    @Autowired
    @Qualifier("sessionFactoryB2eDv")
    protected SessionFactory sessionFactoryB2e;

    @SuppressWarnings("unchecked")
    public B2eDvHibernateDao() {
        persistentClass = (Class<T>) ((ParameterizedType) getClass().getGenericSuperclass()).getActualTypeArguments()[0];
    }

    protected Session getCurrentSession() {
        return sessionFactoryB2e.getCurrentSession();
    }
