 ...
 * @author hendra.fs
 */
public class WfsHibernateDao<T, K extends Serializable> implements WfsDao<T, K> {

    @SuppressWarnings("unchecked")
    protected Class persistentClass;

    @Autowired
    @Qualifier("sessionFactoryWfs")
    protected SessionFactory sessionFactoryWfs;

    @SuppressWarnings("unchecked")
    public WfsHibernateDao() {
        persistentClass = (Class<T>) ((ParameterizedType) getClass().getGenericSuperclass()).getActualTypeArguments()[0];
    }

    protected Session getCurrentSession() {
        return sessionFactoryWfs.getCurrentSession();
    }
