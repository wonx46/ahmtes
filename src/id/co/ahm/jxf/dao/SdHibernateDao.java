 ...
 * @author achmad.ha
 */
public class SdHibernateDao<T, K extends Serializable> extends BaseColumnFilterDao implements SdDao<T, K> {

    @SuppressWarnings("unchecked")
    protected Class persistentClass;

    @Autowired
    @Qualifier("sessionFactorySd")
    protected SessionFactory sessionFactorySd;

    @SuppressWarnings("unchecked")
    public SdHibernateDao() {
        persistentClass = (Class<T>) ((ParameterizedType) getClass().getGenericSuperclass()).getActualTypeArguments()[0];
    }

    protected Session getCurrentSession() {
        return sessionFactorySd.getCurrentSession();
    }
